...
# robot.set_speed_percent(100) # 设置速度百分比
# robot.set_installation_pos(robot.OPTIONS.INSTALLATION_POS.HORIZONTAL) # 设置安装姿态
# robot.set_motion_mode(robot.OPTIONS.MOTION_MODE.J) # 设置运动模式J表示关键模式
...
# robot.move_p([0.1, 0.0, 0.3, 0.0, 1.570796326794896619, 0.0]) # move_p点到点1的笛卡尔位姿运动[x, y, z, roll, pitch, yaw]
# wait_motion_done(robot, timeout=5.0)

# robot.move_l([0.2, 0.0, 0.3, 0.0, 1.570796326794896619, 0.0]) # move_l表示沿直线移动到指定笛卡尔位姿。
# wait_motion_done(robot, timeout=5.0)
...
# robot.move_c(start_pose, mid_pose, end_pose) # move_c表示三个位置确定的圆弧移动
# wait_motion_done(robot, timeout=5.0)

# robot.move_j([0.0] * robot.joint_nums) # move_j直接控制每个关节的角度
# wait_motion_done(robot, timeout=5.0)
...
# for i in range(1, robot.joint_nums + 1): # 逐关节控制目标位置，目标速度，位置刚度系数。速度阻尼系数，前馈力矩
#     robot.move_mit( 
#         joint_index=i,
#         p_des=0.0,
#         v_des=0.0,
...
# robot.move_j([0] * robot.joint_nums) # 回零点
# time.sleep(2)

# robot.electronic_emergency_stop() # 触发急停
# time.sleep(1)
...
while True:
    ...
    print("arm_status:", robot.get_arm_status())
    print("joint_angles:", robot.get_joint_angles())
    print("enable_status:", robot.get_joints_enable_status_list())

    for i in range(1, 7):
        print(f"J{i} driver:", robot.get_driver_states(i))
        print(f"J{i} motor :", robot.get_motor_states(i))
    ...
    # print(robot.fk([0.0] * robot.joint_nums)) # 正向运动学，通过关节角度计算末端位姿
...
# Calibrate the gripper. 校准手爪
# 1. Disable the gripper.
# end_effector.disable_gripper()

# 2. Set the actual maximum stroke of the gripper.
# end_effector.set_gripper_teaching_pendant_param(max_range_config=0.07)  # 0.07m 最大行程
...
# end_effector.move_gripper_m(0.07) # 按米来控制
# time.sleep(0.2)
# end_effector.move_gripper_deg(0) # 看旋转角度来控制
